...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // This is the progress that actually drives the animation, smoothed via lerp.
    ...
    const tick = () => {
      ...
      // Top-half Sienna layout:
      // Start: big word pinned to the top half, clipped by container.
      // End: small logo at top-left of viewport.
      const navH = 56;
      const startLeftPx = 0;
      ...
      // Translate is used to fine-tune the big title position in its clipped container.
      // At start, keep it flush; at end, no translate needed.
      const targetTranslateXPx = 0;
      const targetTranslateYPx = 0;
      const targetScale = 1 + (0.07 - 1) * eased; // 1 -> 0.07

      // Style values update with smoothing so it stays ultra smooth.
      ...
      animRef.current.translateXPx = lerp(
        animRef.current.translateXPx,
        targetTranslateXPx,
        0.08
      );
      animRef.current.translateYPx = lerp(
        animRef.current.translateYPx,
        targetTranslateYPx,
        0.08
      );
    ...
    // Initialize to the Sienna top-half layout immediately
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <main id="top" className="relative z-10">
        {/* Hero: top-half title + bottom-half image triptych */}
        <section className="relative h-screen bg-black">
          {/* Top half: clipped title */}
          ...
          {/* Bottom half: three images */}
          <div className="absolute left-0 right-0 bottom-0 h-[45vh] px-1">
            <div className="h-full flex gap-[4px]">
              {heroImageError.hero1 ? (
                <div className="w-[33.33vw] h-full bg-[#E61A1A] flex items-center justify-center text-white font-semibold uppercase tracking-[0.25em]">
                  Jouets
                </div>
              ) : (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={publicImageUrl(HERO_IMAGES.hero1)}
                  alt="Garçon avec sac à dos"
                  className="w-[33.33vw] h-full object-cover"
                  onError={(e) => {
                    e.currentTarget.style.background = "#1a1a1a";
                    setHeroImageError((prev) => ({ ...prev, hero1: true }));
                  }}
                />
              )}

              {heroImageError.hero2 ? (
                <div className="w-[33.33vw] h-full bg-[#1A1AE6] flex items-center justify-center text-white font-semibold uppercase tracking-[0.25em]">
                  Scolaire
                </div>
              ) : (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={publicImageUrl(HERO_IMAGES.hero2)}
                  alt="Fille qui peint"
                  className="w-[33.33vw] h-full object-cover"
                  onError={(e) => {
                    e.currentTarget.style.background = "#1a1a1a";
                    setHeroImageError((prev) => ({ ...prev, hero2: true }));
                  }}
                />
              )}

              {heroImageError.hero3 ? (
                <div className="w-[33.33vw] h-full bg-[#F5A623] flex items-center justify-center text-white font-semibold uppercase tracking-[0.25em]">
                  Bureautique
                </div>
              ) : (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={publicImageUrl(HERO_IMAGES.hero3)}
                  alt="Enfants avec jouets"
                  className="w-[33.33vw] h-full object-cover"
                  onError={(e) => {
                    e.currentTarget.style.background = "#1a1a1a";
                    setHeroImageError((prev) => ({ ...prev, hero3: true }));
                  }}
                />
              )}
            </div>
          ...
          <div className="overflow-hidden">
            <div className="whitespace-nowrap py-3 text-sm font-medium tracking-[0.25em] uppercase flex">
              <div className="animate-[ticker_25s_linear_infinite] flex">
                <span className="mx-6">
                  Jouets ★ Scolaire ★ Bureautique ★ Beaux Arts ★ Livres ★ Sac à
                  dos ★
                </span>
                <span className="mx-6">
                  Jouets ★ Scolaire ★ Bureautique ★ Beaux Arts ★ Livres ★ Sac à
        ...
        <section
          id="products"
          className="px-6 md:px-8 lg:px-12 py-20 space-y-10 bg-gradient-to-b from-[#0a0a0a]/80 via-black/90 to-[#050505]"
        >
          <div className="max-w-6xl mx-auto">
            ...
            <p className="text-sm text-white/70 max-w-xl">
              Une sélection cinématographique de jouets, fournitures scolaires,
              beaux-arts et plus encore — prête à être commandée par WhatsApp.
            </p>
          </div>
          ...
          <div className="max-w-6xl mx-auto">
            {loading ? (
              ...
              <div className="max-w-md bg-red-900/40 border border-red-500/40 text-red-100 text-sm rounded-2xl px-4 py-3">
                {error}
              </div>
            ) : products.length === 0 ? (
              <div className="max-w-md bg-white/5 border border-white/10 rounded-2xl px-6 py-10 text-center text-sm text-white/70">
                Aucun produit pour le moment. Ajoutez vos premiers articles
        ...
        <section
          id="categories"
          className="px-6 md:px-8 lg:px-12 py-20 bg-[#050505]"
        >
          <div className="max-w-6xl mx-auto space-y-8">
            <div>
              ...
              <p className="text-sm text-white/70 max-w-xl">
                Un univers d&apos;écriture, de jeu et de création pour chaque
                âge.
              </p>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-4 auto-rows-[minmax(160px,1fr)]">
              {[
                ...
                <div
                  key={cat.name}
                  className={`relative overflow-hidden rounded-2xl bg-[#111] border border-white/5 p-4 flex flex-col justify-between group cursor-pointer transition-transform duration-300 hover:scale-[1.05] hover:shadow-[0_18px_45px_rgba(0,0,0,0.75)] ${
                    cat.span || ""
                  }`}
                  style={{
                    boxShadow:
                      "0 0 0 0 rgba(0,0,0,0.3), 0 20px 40px rgba(0,0,0,0.7)",
                  }}
                >
                  <div
                    className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                    style={{
                      background: `radial-gradient(circle at top left, ${cat.color}33, transparent 60%)`,
                    }}
                  />
                  ...
                  <div
                    className="relative z-10 mt-4 h-[1px] w-16"
                    style={{
        ...
        <section
          id="about"
          className="px-6 md:px-8 lg:px-12 py-20 bg-gradient-to-b from-[#050505] to-black"
        >
          <div className="max-w-6xl mx-auto space-y-10">
            <div className="space-y-3">
              <p className="text-sm tracking-[0.25em] uppercase text-[#F5A623]">
                الخبرة و التميز — Expérience et Excellence
              </p>
              <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight">
                Oscar Sarl, votre partenaire scolaire &amp; bureautique à
                Tlemcen.
              </h2>
              <p className="text-sm text-white/70 max-w-xl">
                Adresse : N°36 Bataille Felaoucen, Tlemcen — un espace dédié aux
                fournitures scolaires, jouets éducatifs, arts plastiques et
            ...
            <div className="grid gap-6 md:grid-cols-3">
              <div className="bg-[#101010] border border-white/10 rounded-2xl p-5">
                ...
                <p className="mt-2 text-3xl font-extrabold text-[#F5A623]">
                  15+
                </p>
                <p className="mt-1 text-xs text-white/60">
                  Une présence durable auprès des écoles, familles et
                  entreprises.
                </p>
              </div>
              ...
              <div className="bg-[#101010] border border-white/10 rounded-2xl p-5">
                ...
                <p className="mt-2 text-3xl font-extrabold text-[#1A1AE6]">
                  800+
                </p>
                <p className="mt-1 text-xs text-white/60">
                  Jouets, livres, fournitures, bureautique et beaux-arts dans un
                  même lieu.
                </p>
              </div>
              ...
              <div className="bg-[#101010] border border-white/10 rounded-2xl p-5">
                ...
                <p className="mt-1 text-xs text-white/60">
                  Parents, élèves, artistes et entreprises qui nous font
                  confiance.
                </p>
              </div>
            </div>
      ...
      {/* Keyframes for ticker & smooth scroll (Tailwind can't define here) */}
      <style>{`
        @keyframes ticker {
          0% {
